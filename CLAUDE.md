...
- `docs/` contains only the quadrant entries, a `README.md` router, and a single flat `assets/` holding every figure the docs reference (plus site infrastructure when a static-site generator requires it inside the docs dir). No per-quadrant `images/` folders. Planning notes and historical material live outside `docs/`.
...
- `docs-init` surveys a whole project and creates/reconciles all quadrants (and asks before restructuring a repo that follows a different deliberate convention); `docs-update` is the cheap incremental variant, run after a specific change (or against a diff/commit range) to update only the quadrants that change actually touches. Retrofitting an already-documented project to a newer version of this convention is `docs-init`'s reconcile mode, not `docs-update` — reconcile runs dedicated audit passes over naming, visuals, and redundancy across every page; `docs-update` only ever touches what the change touched, and reports rather than fixes anything stale or duplicated outside that scope.
