...
- Each quadrant is either a flat `docs/<quadrant>.md` or, once it outgrows one topic, a directory `docs/<quadrant>/` with an `index.md` + one file per topic. Promote flat→directory only when content genuinely warrants it; never drop content in the process.
...
- Mermaid diagrams for architecture/flow belong in `explanation`.
- Relative links only — never absolute URLs back to a repo — so docs can later be pulled into a centralized cross-project doc portal.
- Never invent facts, flags, or behavior not actually present in the code/config; mark genuine gaps with `<!-- TODO: verify -->`.
- `docs-init` surveys a whole project and creates/reconciles all quadrants; `docs-update` is the cheap incremental variant, run after a specific change (or against a diff/commit range) to update only the quadrants that change actually touches.

`problem-brainstorming.md` at the repo root captures the original design conversation behind this convention and the `docschemy` repo itself — useful background if extending the convention, not something to keep in sync with code.
