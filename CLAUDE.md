...
The four skills currently in this repo (`docs-init`, `docs-update`, `docs-edit`, `docs-search`) implement a documentation convention this user applies across *other* projects — worth understanding since it's the repo's reason for existing, and since this repo's own docs follow the same convention:
...
- The other two skills split off the same convention by **trigger**, which also fixes what each may treat as authoritative. `docs-edit` handles changes a *person* wants with no code change behind them — style, restructuring, "always write it this way" — under the rule that **the user decides how the docs read, the repo decides what they say**: every factual claim in a request is verified against the code before it's written, and a contradicted claim is reported rather than written. It also maintains the preferences layer: a `## Docs preferences` block in the project's `CLAUDE.md` (or `~/.claude/CLAUDE.md` for cross-project ones) that all four skills read at the start of a run — style overrides freely, structural overrides only when recorded with their reason, and nothing may override verification. Without that block, an unrecorded deviation is indistinguishable from drift and the next `docs-update` reverts it. `docs-search` answers a question from the docs and **never writes**, which is what makes it safe mid-task; it searches audience-first (notably "why" → `internals/adrs/`), cites the page and heading behind each answer, never cites a page it didn't open, and reports which of three outcomes it hit — the docs answer it, the docs are silent, or the docs contradict the code. Being read-only it doesn't require the convention, so it works on any layout.
